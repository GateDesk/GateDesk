...
/// The session panel's title bar.
///
/// A Sciter `SW_MAIN` window destroys itself, and with itself the process, the moment its
/// close button is used, and nothing the document can raise calls that off: the documented
/// way to refuse is an `evt.data = "cancel"` on the close request, which
/// `sciter::EventHandler::on_event` is given no handle on. The window is therefore subclassed
/// and the close button answered one level below Sciter - except with shift held, which is
/// left to Sciter and ends the process.
#[cfg(windows)]
mod panel_window {
    ...
    pub fn tame_title_bar(hwnd: isize) {
        ...
        unsafe {
            // `SetWindowLongPtrW` is a 64-bit name; on a 32-bit build the `windows` crate maps
            // it onto `SetWindowLongW`, whose value is the narrower one.
            #[cfg(target_pointer_width = "64")]
            let sciter_proc = SetWindowLongPtrW(hwnd, GWLP_WNDPROC, panel_proc_address as isize);
    ...
    /// The close button minimizes instead of closing the window, because the panel is the only
    /// place the peer that is connected right now can be answered. Holding shift is what takes
    /// the process down.
    unsafe extern "system" fn panel_proc(hwnd: HWND, msg: u32, wp: WPARAM, lp: LPARAM) -> LRESULT {
        if is_close_request(msg, wp) && !shift_held() {
            let _ = ShowWindow(hwnd, SW_MINIMIZE);
            return LRESULT(0);
        }
        CallWindowProcW(sciter_proc(), hwnd, msg, wp, lp)
    }

    /// The close button arrives twice: the button itself reports `SC_CLOSE`, and the default
    /// window procedure turns that into a `WM_CLOSE`.
    fn is_close_request(msg: u32, wp: WPARAM) -> bool {
        msg == WM_CLOSE || (msg == WM_SYSCOMMAND && (wp.0 as u32 & 0xFFF0) == SC_CLOSE)
    }

    /// Read as the message is handled rather than when the button was clicked, which is what
    /// makes it the state the user made the click in: a busy queue can delay us, but the key
    /// state of a queued message is the one that was true when the queue took it in.
    fn shift_held() -> bool {
        unsafe { GetKeyState(VK_SHIFT.0 as i32) < 0 }
    }

    /// Sciter's own window procedure, as it was when the window was subclassed.
    fn sciter_proc() -> WNDPROC {
        unsafe { std::mem::transmute(SCITER_PROC.load(Ordering::SeqCst)) }
    }
}
