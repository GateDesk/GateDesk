...
| 本机凭据 | 被控端 | `/password` | 本进程（§6.5）—— 它设的是**本机自己的**连接密码，本机是要被连进来的那一端 |
...
> 同一台机器可以**同时是控制端和被控端**（一边连着别人，一边被别人连），所以每个接口都要按「这次调用管的是哪一端」来读。
...
- **「本机」= 调用这个 API 的那台机器**。控制端小节里的「本机」指发起连接的那一边，被控端小节里指被连进来的那一边；同一台机器可能同时是两者，所以每节都按「这一节管的是哪一端」来读。
...
**审计（v1.7）**：调用成功即视为客户对本机执行了授权动作（方式 A——生成连接凭据），写入操作级审计事件 `auth.grant`（失败写 `auth.grant` result=`err`）；连同 `connect.start` / `connect.close` 等事件，见 §6.8。
...
本节接口驱动的是**本机作为被控端**的会话。这些状态由连接管理器进程持有，本 API 通过进程内通道（`_cm`）让它执行动作 —— 与人在受控端界面上点按钮是**同一批动作**，因此接口与界面不会出现两种状态：用接口批准后，界面上对应的提示会同步消失。

| 接口 | 干什么 | 前提 | 成功响应 |
|------|--------|------|---------|
| `GET /sessions` | **看**：现在有哪些对端、各自当前能做什么、有没有正在等本机应答的申请 | 连接管理器在跑 | `{"ok":true,"sessions":[…]}` |
| `POST /approve` | 把正在敲门的对端**放进来**，或拒绝它 | 对端正在等批准 | `{"ok":true,"result":{"peer_id":…}}` |
| `POST /control` | **应答对端的一项申请**：键鼠控制，或某一项命名权限 | 有 `pending_control` | 同上 |
| `POST /permission` | 不经申请，**直接开关**对端的某一项权限 | 会话已建立 | 同上 |
| `POST /terminate` | **结束会话**（= 界面上的「断开」，对端可重连） | 会话在 | 同上 |
| `POST /dismiss` | 把**已结束**的会话从列表里清掉（=「关闭」） | 会话确已结束 | 同上 |

> 设计前提：接入后**默认只读**，控制端要操作本机键鼠必须由本机同意 —— 界面上有人点，或本节接口代替人做决定。
>
> **因此：持有 token 即可代替本机用户批准接入与控制。** token 的保管按 §2 执行。
>
> **本节的状态变化只能靠轮询发现**：本机 API 没有推送通道（`tiny_http` 同步模型）。要降低发现延迟见 §6.10 的出站事件通知（在被控机配 `event-server-url` 后生效）；**没配时轮询就是唯一方式**。

**错误码怎么读**（各小节的「什么时候会失败」是精确条件，这里只说规律）

- `404`：`id` 对应的会话不存在（写错了，或已被 `/dismiss` 清掉）。
- `409`：会话**在**，但当前状态做不了这件事。
- `503`：本机没有连接管理器进程在监听 `_cm`（既没有会话，也没有那个窗口）。
- `504`：连接管理器 2 秒没回应。
- `400`：参数缺失或非法。

> **超时与重试**：2 秒没回应时动作可能已经在本机生效。重试是安全的：`/approve` 在**已经放行**时重复得到 409（放行过后才有这个结果；被拒的对端条目在清掉前仍是「未放行」，重复调只是再关一次，返回 200），`/terminate` 重复得到 200（空操作），`/permission` 重复只是把同一个值再写一遍，`/dismiss` 重复会 404。不确定就先查 `/sessions`。
>
> **对已经结束的会话**（`disconnected: true`）只有 `/dismiss` 会因状态不符报 409；`/approve`、`/control`、`/permission`、`/terminate` 都仍返回 200 —— 它们的前置条件（是否放行 / 是否有待应答申请 / 会话是否存在）都不看 `disconnected`。所以 **200 不代表会话还活着，要判活看 `/sessions` 的 `disconnected`**。
...
**干什么**：返回受控端界面上正显示的内容 —— 每个已接入或正在请求接入的对端，及其权限、是否等待批准、是否有待应答的申请。轮询它就知道该做什么（§8.1 的无人值守流程就是这么写的）。
...
| keyboard | bool | **对端当前能不能驱动本机键鼠**：权限与会话批准两样都成立才为 `true`，即控制层 `peer_input_enabled()` 的值。与连接管理器界面上键盘那一行是同一个值 —— 界面写「允许」就必须真的能控制，否则现场的人无从判断（v1.14） |
| peer_id | string | 对端设备 ID |
| name | string | 对端名称 |
| other fields | — | 各项权限的当前值，与界面上的开关一一对应 |

**什么时候会失败**：本机没有连接管理器进程在监听 `_cm`（既没有会话，也没有那个窗口）→ 503；它 2 秒没回应 → 504。只要它在监听就返回数组 —— 一个会话都没有时是 `[]`，会话刚结束、条目还没清掉时那些条目仍会出现（`disconnected: true`）。它不接受 `id` 参数，所以**永远不会 404**。
...
**干什么**：对正在敲门（`/sessions` 中 `authorized: false` 且 `disconnected: false`）的对端做决定 —— `true` 放它进来，`false` 拒绝并断开。无人值守时这是会话能开始的唯一途径：本机不开 `--ui` 时，那个窗口是唯一在听对端登录请求的地方，平台不代为应答就只能由人点。
...
**什么时候会失败**：`id` 或 `accept` 缺失/非法 → 400；会话不存在 → 404；**该对端已经放行过** → 409 `the peer is already through the door`。
...
**干什么**：应答对端发来的一项申请，本机是决定方。`/sessions` 里 `pending_control: true` 时走这里 —— 两种申请共用这一个入口，用 `pending_permission` 区分：

- **`pending_permission` 为空**：对端要的是**键鼠控制**。`true` 交出控制（会话从只读变成可操作），`false` 拒绝、会话保持只读。
- **`pending_permission` 非空**：对端要的是**那一项权限**（`clipboard` / `audio` / `file`）。`true` 时本机**替本机用户把那项权限打开**（与 §6.7.4 是同一批动作），`false` 则什么也不变。
...
**超时**：对端的申请有 **60 秒**时限，无人应答由服务端按拒绝处理，会话保持原样 —— **静默不等于同意**。超时记为 `control.timeout`。
...
**另一道门**：本接口不是唯一的决定入口。键鼠那一项，本机用户在连接管理器界面上点开键盘图标同样是「同意控制」（上游一直就是这个语义，本客户端保持它），关掉图标则是收回控制（§6.7.4）。两条路落到同一处，所以状态与审计不会出现两种说法。
...
**干什么**：不经任何申请，直接开关本机允许对端做的一件事 —— 会话已经建立时用它调整；由对端开口要的那条路在 §6.7.3 与 §6.9。开关的是**本机**的态度，落点在连接层，与人在受控端界面上拨同一个开关是同一批动作。
...
- **这是本接口与会话界面的边界，不是本机用户本人的边界**：`--ui` 打开的原始 CM 窗口仍是上游那八个开关（含远程重启 / 录制 / 阻止输入 / 隐私模式），由坐在机器前的人自己切换，两者不冲突。界面有两条路径：`--ui` = 原始 CM 窗口（`cm.tis`，上游原样），非 `--ui`（`--gd-panel`）= 定制界面（`cm_sh.tis`，由原始 CM 窗口复制而来，只画 A 类四项并接对端申请提示）。
- 这四项在会话建立时都是关闭的。对端可以开口要，但要不到：申请只是把「有人想开」摆到本机用户眼前（§6.9），**答案始终由本机用户给**。给答案的路总共三条 —— 人拨开关、本接口、以及 §6.7.3 应答对端的申请 —— 三条最终落到同一段开关逻辑（本接口与「应答命名权限申请」都调 `switch_permission`），所以状态不会出现两种说法。
- **本接口没有「必须有一个在途申请」这个前提**，这与 §6.7.3 不同：`/control` 只在真有人申请时才能调（否则 409），本接口任何时候都能调。也就是说本接口是**单方面决定**，等价于本机软件代用户授权 —— 谁持有 token，谁就能在电脑前没人的情况下替本机用户把键鼠交出去。
- 运维设置 `enable-perm-change-in-accept-window = N`（锁定权限）时，本接口除 `keyboard` 外一律 409；这道锁也管着 §6.9 里**命名权限**的应答（`respond_control_request` 自己会检查）。**但它管不住 `keyboard`**：`keyboard` 在门口因 `name != "keyboard"` 免于 409，而 §6.7.3 应答键鼠申请走的路径（`resolve_control_request`）根本不经过 `switch_permission`。把 `keyboard` 排除在外是有意的（它是 `--ui` 形态下现场唯一的放行门），但运维需要知道：**「锁权限」锁不住键鼠**。
- 打开 `keyboard` **就是**授权控制：本机用户点开键盘图标（或本接口打开 `keyboard`）与 §6.7.3 应答一次键鼠申请等价，两者都写同一个闸门；关掉 `keyboard` 则收回控制，会话退回只读。区别只在「谁来点」—— 闸门本身是一处。
...
> ⚠️ **`actor` 恒为 `customer`**：事件在连接层记录，人拨开关、§6.7.3 应答、本接口调用三条路写出来**一模一样**，日志里分不出是现场的人做的还是本机软件代做的。平台代授权时需自行留日志，两边靠 `session_id` + 时间对账。
...
**干什么**：结束一个会话 —— 与受控端界面上的「断开」一致。对端会知道是被本机用户结束的，因此允许重连。
...
**干什么**：把已经结束（`/sessions` 中 `disconnected: true`）但还留在列表里的条目移除 —— 与受控端界面上的「关闭」一致。它连带做收尾（例如释放该会话占用的剪贴板文件），所以比「从列表里删一行」要多一点。
...
| 受控端闸门 | §6.7.3 的会话控制闸门；同意 = 交出键鼠控制，关掉即收回 | §6.7.4 的权限开关本身；同意 = **受控端本机**替用户调一次那个开关，与他在界面上手动拨开关是同一批动作 |
| 结果可见性 | 对端同意后发 `Permission::Keyboard`，控制端立刻能看出只读解开 | 没有回执，只能从通道是否真的可用反推（剪贴板能不能用、有没有声音） |
| 审计（记在**受控端**） | `control.approve` / `control.deny` / `control.timeout` | 允许 → `permission.change`（`extra.name` 带权限名，`extra.enabled` 为 `true`）；超时 → `control.timeout`；请求本身不记事件 |
...
> **实现位置**：上报侧在 `GateDesk/src/event.rs`，由连接层四个点调用（对端等批准、对端申请、会话建立、会话结束）；接收侧在 `GateDeskWeb/server.js` 的 `POST /api/event`，收到后经 WebSocket 广播给该设备房间里的页面（`admin.html` / `employee.html` 的 `data.type === 'event'` 分支）。
>
> 所有这些都以配置键 `event-server-url` 为总闸：**为空时四个点什么也不发**，行为与没有这个机制时完全一致。

**它解决什么问题**

§6.7 那一组只能靠轮询发现变化，而轮询有两个固有缺陷：

1. **发现延迟**：一次「有人申请键鼠 / 权限」只有 60 秒窗口（`CONTROL_REQUEST_TIMEOUT`），轮询慢了就永久错过，对端只能重新发起；
2. **代价随设备数增长**：每台被控机都要一个定时器 + 一份 token，`N` 台就是 `N` 份运维对象。

本机制让被控端在**状态刚变**时主动 POST 一次，平台从「定时器」变成「事件驱动 + 兜底轮询」。

**配置**：`[options] event-server-url`（可空，默认空 = 关闭，见附录 A.3）。为空时行为与现在完全一致。
**投递约定**

| 项 | 约定 |
|----|------|
| 方向 | 被控端 → 平台，出站 POST。与 §6.8 的审计转发同向，但**独立配置、独立端点** |
| 方法 / 头 | `POST <event-server-url>`，`Content-Type: application/json` |
| 超时 | 3 秒 |
| 重试 | **不重试**。丢了就丢了，靠兜底轮询补上 |
| 鉴权 | 桌面端**不加**任何头（与 `audit-server-url` 现状一致）。平台侧鉴权由 URL 自带，如 `?key=<secret>` |
| 应答 | 平台返回任意 2xx 即视为收到，其余状态码不影响桌面端 |
| 语义 | **这是一句「去看一眼」的提示，不是状态本身，也不是记录。** 状态的真源永远是 `GET /sessions`（§6.7.1），记录的真源永远是 §6.8 |
...
| session_id | int | 会话标识，即 `GET /sessions` 返回的 `id`（§5 的寻址约定），可拿去直接调 §6.7 的动作 |
...
| `control.pending` | 出现一个待应答的申请（`pending_control` 变成 `true`） | `{"permission":""}` = 键鼠；`{"permission":"clipboard"}` 等 = 命名权限 | 按平台策略 `POST /control`（§6.7.3）；键鼠要慎重，见 §6.9 影响面提示 |
...
> 事件名（`event`）与 §6.8 的审计 action 名是**两套命名空间**，故意不统一：审计的 action 描述「发生了什么」（如 `session.terminate`），事件名描述「要你去看一眼什么」（如 `control.pending`）。所以 `session.open` / `session.close` 在 §6.8 里没有同名事件，别拿它去对账。

**平台必须遵守的两条**

1. **幂等，且以 `/sessions` 为准**：同一次变化可能通知多次，也可能一次都不通知。收到后**先查一次 `/sessions` 再动作**，不要拿通知当状态用 —— 通知到达时那个申请可能已经被现场的人点了，或已经超时了。
2. **兜底轮询不能省**：建议 30 秒一轮。事件通知**不是**可靠性机制，它只负责把发现延迟从「一个轮询周期」降到「事件到达即返回」。
...
> 两者可以指向同一个平台，但**建议用不同路径**（如 `/api/audit` 与 `/api/event`），别把「提示」和「记录」混在一个端点上。
...
### 8.1 被控端无人值守批准（v1.8；v1.19 增加事件驱动选项）

设备不开 `--ui` 时，会话面板是唯一能应答对端请求的窗口。若希望由平台统一决策（而非现场有人点），平台要做两件事：**发现**，然后**决策**。
...
| 方式 | 延迟 | 代价 | 何时用 |
|------|------|------|--------|
| 只轮询 | ≤ 一个轮询间隔 | 每台设备一个定时器 | 设备少，或平台已有统一轮询框架 |
| 事件通知 + 兜底轮询（§6.10） | 约一个网络往返 | 每台设备配一个 `event-server-url` | 设备多，或对 60 秒窗口敏感 |

两种方式**都需要兜底轮询** —— §6.10 的通知不保证送达，也不保证只送一次。

> 参考实现：`admin.html` / `employee.html` 先按 **2 秒**兜底轮询（轮询即全部），**一旦真收到过一条事件**就说明通知链路是通的，兜底随即放到 **30 秒**。页面无从知道对面配没配，所以只能这样自适应；纯轮询集成则建议不超过 5 秒一轮。

- **轮询间隔必须显著小于 60 秒**：控制请求超时即被拒绝，间隔太大会错过窗口、控制端只能重新发起。
- **收到通知后先查 `/sessions` 再动作**：通知到达时那个申请可能已经被现场的人点了，或已经超时。
- 平台代为批准会在本机 `audit.log` 留下 `login.approve` / `control.approve` 等记录，但**不会注明是哪个平台**（`actor` 恒为 `customer`，见 §6.7.4）—— 平台侧需自行留日志，两边靠 `session_id` + 时间对账。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-09-21 | 1.20 | **§6.10 出站事件通知已落地**（不再是设计稿）：新增 `GateDesk/src/event.rs`（独立配置键 / 独立队列 / 专用线程 / **不重试、无本地兜底、3 秒硬上限**），在连接层四个点发出 `login.pending` / `control.pending` / `session.open` / `session.close`；`GateDeskWeb` 新增 `POST /api/event` 接收并广播，两个页面收到即拉 `/sessions`、兜底轮询由 2 秒自适应放到 30 秒（收到过事件才放）。`session.close` 的 `extra` 补上 `reason`。附带修掉 `employee.html` 里还在调已删除的 `POST /voice` 的语音按钮（改为按会话的 `POST /permission {"name":"audio"}`）。 |
| 2026-09-21 | 1.19 | **新增 §6.10 出站事件通知**（配置键 `event-server-url`，**尚未实现**，先定契约）：受控端在出现等批准的对端 / 待应答的申请 / 会话开始结束时主动 POST 一次，把 §6.7 的发现延迟从「一个轮询周期」降到「事件到达即返回」；§8.1 相应改为「事件驱动 + 兜底轮询」两种发现方式并列。同时修正四处与实现不符：§1 角色表把 `/password` 归错了端（它设的是**本机**密码，属于被控端，拆为独立一行）、§1 端点清单还留着已删除的 `/voice`、§6.9 差异表写 `extra.permission` 而实际字段是 `extra.name`、头部维护约定与 §1 的源码路径写成小写 `gatedesk/`。另补 §6.7.4 三条事实：本接口没有「必须有在途申请」这个前提（单方面决定）、`enable-perm-change-in-accept-window = N` **管不住 `keyboard`**（它在门口免于 409，应答键鼠申请的路径也不经 `switch_permission`）、`permission.change` 的 `actor` 恒为 `customer` 因此分不出是谁发起的。 |
