...
向一个**已经打开**的会话的对端要键鼠控制权。这是 §6.7.3 的对侧：本机主动请求，对端自己弹确认界面应答。
...
```powershell
curl -X POST "http://127.0.0.1:21120/request-control?token=<token>" -d "{\"id\":\"123456789\"}"
```
...
**语义**：与远程窗口菜单里的「请求控制」是同一件事 —— 把 `disable_keyboard` 选项清掉，告诉对端「我们要键鼠」。连接建立在会话进程里，而本 API 运行在 `--server` 进程，所以请求经**进程内通道**投递给该对端的会话进程，并按对端 ID 寻址 —— 同一台机可以同时开多个会话，各自有自己的监听，不会串。

**200 只表示会话已受理，不代表对端已同意**：对端的 GateDesk 会弹「是否允许…控制您的鼠标键盘」， 60 秒内无人应答按拒绝处理，期间会话保持只读（§6.7.3）。同意后对端发 `Permission::Keyboard`，控制端才解开只读。
