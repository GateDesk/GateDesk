...
// GateDesk patch: resolve the target's own cfgs at run time instead of with `#[cfg]`.
//
// `#[cfg]` here describes the machine that *compiles the build script*, which is always the
// host. Cross compiling x64 -> x86 therefore selected the x64 prebuilt library and the link
// failed with "LNK4272: library machine type 'x64' conflicts with target machine type 'x86'".
// GateDesk ships x64 and x86 and builds both from one machine, so the target decides. The
// values come from the environment cargo exports to build scripts and mirror the `#[cfg]`s
// they replace.
//
// A 64 bit non-x64 msvc target still selects the x64 library, as before: the crate ships no
// other prebuilt one, and the aarch64 CI job avoids this function entirely by pointing
// SODIUM_LIB_DIR at a vcpkg build (handled earlier, in `find_libsodium_env`).
#[allow(dead_code)]
fn target_env_name() -> String {
...
fn get_lib_dir() -> PathBuf {
    let bits = target_pointer_width();
    if target_env_name() == "msvc" {
        let arch = if bits == "32" { "Win32" } else { "x64" };
        let profile = if is_release_profile() {
            "Release"
        } else {
            "Debug"
        };
        return get_crate_dir().join(format!("msvc/{}/{}/v142/", arch, profile));
    }
    let arch = if bits == "32" { "win32" } else { "win64" };
    get_crate_dir().join(format!("mingw/{}/", arch))
}
