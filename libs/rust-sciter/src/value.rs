...
{
	data: VALUE,
	// `UnsafeCell` because the scratch value has to be allocated on first use from
	// `Index::index`, which only gets `&self`. Writing the pointer through a shared
	// reference is what the previous `&self as *const _ as *mut _` cast did, and
	// that is undefined behaviour which current Rust rejects outright.
	tmp: ::std::cell::UnsafeCell<* mut Value>,
}
...
impl Value {
	...
	pub const fn new() -> Value {
		Value { data: VALUE::new(), tmp: ::std::cell::UnsafeCell::new(::std::ptr::null_mut()) }
	}
  ...
  ///
	...
	fn ensure_tmp_mut(&self) -> &mut Value {
		let slot = self.tmp.get();
		let mut p = unsafe { *slot };
		if p.is_null() {
			p = Box::into_raw(Box::new(Value::new()));
			unsafe { *slot = p };
		}
		return unsafe { &mut *p };
	}
...
impl Drop for Value {
	fn drop(&mut self) {
		...
		let tmp = unsafe { *self.tmp.get() };
		if !tmp.is_null() {
			let _drop_tmp = unsafe { Box::from_raw(tmp) };
		}
