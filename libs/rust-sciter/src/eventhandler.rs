...
fn process_events(me: &mut dyn EventHandler, he: HELEMENT, evtg: UINT, params: LPVOID) -> BOOL
{
	...
	{
		eprintln!("[sciter] warning! null element for {:04X}", evtg.0);
	}
	...
	let result = match evtg {
      ...
      use dom::event::EventReason;
			let code = BEHAVIOR_EVENTS(nm.cmd & 0x0_0FFF);
			let phase = PHASE_MASK(nm.cmd & 0xFFFF_F000);
			let reason = match code {
				BEHAVIOR_EVENTS::EDIT_VALUE_CHANGED | BEHAVIOR_EVENTS::EDIT_VALUE_CHANGING => {
					let reason = EDIT_CHANGED_REASON(nm.reason as UINT);
				...
				_ => {
					let reason = CLICK_REASON(nm.reason as UINT);
    ...
    EVENT_GROUPS::HANDLE_METHOD_CALL => {
      ...
      let nm = unsafe { & *scnm };
      let code = BEHAVIOR_METHOD_IDENTIFIERS((*nm).method);
      ...
      let handled = {
        ...
        let reason = match code {
          BEHAVIOR_METHOD_IDENTIFIERS::DO_CLICK => {
            MethodParams::Click
          },
      ...
      handled
		...
		_ => {
			eprintln!("[sciter] warning! unknown event group {:04X}", evtg.0);
			false
		},
	};
