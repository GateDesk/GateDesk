...
///
/// A bit set, not an enum: the states are combined with `|` and `&`, and an enum
/// cannot hold a combination that is not one of its variants. The previous
/// enum-based version OR'ed/AND'ed through `mem::transmute`, which is undefined
/// behaviour for an unmapped discriminant and now aborts at runtime.
///
