...
/// Window flags.
///
/// A bit set, not an enum: these values are only ever used in combinations, and
/// an enum cannot hold a combination that is not one of its variants. The
/// previous enum-based version built the OR'ed value with `mem::transmute`,
/// which is undefined behaviour for any unmapped discriminant and is now a hard
/// abort at runtime as well - so a main window (SW_MAIN | SW_TITLEBAR |
/// SW_RESIZEABLE | SW_CONTROLS) could not be created at all.
#[repr(transparent)]
#[derive(Copy, Clone, Debug, PartialEq, Eq, Hash)]
pub struct SCITER_CREATE_WINDOW_FLAGS(pub UINT);

#[allow(non_upper_case_globals)]
impl SCITER_CREATE_WINDOW_FLAGS {
	/// child window only, if this flag is set all other flags ignored.
	pub const SW_CHILD: Self = Self(1);
	/// toplevel window, has titlebar.
	pub const SW_TITLEBAR: Self = Self(1 << 1);
	/// has resizeable frame.
	pub const SW_RESIZEABLE: Self = Self(1 << 2);
	/// is tool window.
	pub const SW_TOOL: Self = Self(1 << 3);
	/// has minimize / maximize buttons.
	pub const SW_CONTROLS: Self = Self(1 << 4);
	/// glassy window - "Acrylic" on Windows and "Vibrant" on macOS.
	pub const SW_GLASSY: Self = Self(1 << 5);
	/// transparent window (e.g. `WS_EX_LAYERED` on Windows, macOS is supported too).
	pub const SW_ALPHA: Self = Self(1 << 6);
	/// main window of the app, will terminate the app on close.
	pub const SW_MAIN: Self = Self(1 << 7);
	/// the window is created as topmost window.
	pub const SW_POPUP: Self = Self(1 << 8);
	/// make this window inspector ready.
	pub const SW_ENABLE_DEBUG: Self = Self(1 << 9);
	/// it has its own script VM.
	pub const SW_OWNS_VM: Self = Self(1 << 10);
}
...
/// Flags can be AND'ed.
impl ::std::ops::BitAnd for SCITER_CREATE_WINDOW_FLAGS {
	type Output = SCITER_CREATE_WINDOW_FLAGS;
...
/// Sciter callback notification codes.
///
/// Not an enum: the code arrives from the engine as a plain `UINT` and the
/// numbering has holes (there is no code 0 or 3), so an enum cannot represent
/// every value the engine may hand over. Decoding it with `mem::transmute` was
/// undefined behaviour for such a value and now aborts at runtime.
#[repr(transparent)]
#[derive(Copy, Clone, Debug, PartialOrd, PartialEq, Eq, Hash)]
pub struct SCITER_NOTIFICATION(pub UINT);

#[allow(non_upper_case_globals)]
impl SCITER_NOTIFICATION {
  pub const SC_LOAD_DATA: Self = Self(1);
  pub const SC_DATA_LOADED: Self = Self(2);
  pub const SC_ATTACH_BEHAVIOR: Self = Self(4);
  pub const SC_ENGINE_DESTROYED: Self = Self(5);
  pub const SC_POSTED_NOTIFICATION: Self = Self(6);
	pub const SC_GRAPHICS_CRITICAL_FAILURE: Self = Self(7);
	pub const SC_KEYBOARD_REQUEST: Self = Self(8);
	pub const SC_INVALIDATE_RECT: Self = Self(9);
}
